...
Framework.prototype.myEmit = function (event, ...args) {
  try {
    ...
    if ((args[0] !== 'log') && (this.options.restrictedToEmailDomains)) {
      // TODO pass to emit from membershipRules module
    } else {
...
Framework.prototype.myEmitWithActor = function (event, ...args) {
  try {
    // TODO there may be some events (ie: log) that I want to always pass on
    if ((args[0] !== 'log') && (this.options.restrictedToEmailDomains)) {
      // TODO pass to emit from membershipRules module
    } else {
      const actorId = args.pop();
      this.emit(event, ...args, this.id, actorId);
    }
  } catch(e) {
    if (typeof event != 'string') {
      event = 'UNKNOWN';
