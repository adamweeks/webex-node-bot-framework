...
function Framework(options) {
   ...
   * @property {string} [restrictToEmailDomains] - Set to command seperated list of email domains the bot may interact with, ie "myco.com,mycom2.com".  
        For more details see the [Membership-Rules README](./doc/membership-rules-readme.md)
   * @property {string} [unauthorizedDomainUserEntersResponse] - Message from bot when it is added to a space with users whose emails do not match the 
        list specified (if any) in the `restrictedToEmailDomains` parameter.   Default messages is
   ...
   * @property {string} [unauthorizedDomainStateMessageResponse] - Message from bot when it is messaged in a space with users whose emails do not match the 
        list specified (if any) in the `restrictedToEmailDomains` parameter.   Default messages is
        "Sorry, because my use is not allowed for all the members in this space I am ignoring any input.".
        No message will be sent if this is set to an empty string.
   * @property {string} [unauthorizedDomainUserExitsResponse] - Message from bot when the last user from an unauthorized domain leaves the space.
        The default messages is "I am now allowed to interact with all the members in this space and will no longer ignore any input.".
        No message will be sent if this is set to an empty string.
   * 
   */
  ...
  if (!bot) {
    ...
    if (lockStatusChanged) {
      if (room.isLocked) {
         ...
         * @property {string} id - Framework UUID
         */
        this.emitBoth('roomUnlocked', bot);
      }
    }
...
Framework.prototype.onMembershipCreated = function (membership, actorId) {
  ...
  // else if other membership added to monitored room...
    ...
    if (!(("membershipRules" in this) && (!this.membershipRules.isNewMemberAllowed(bot, membership)))) {
...
Framework.prototype.checkMembershipRules = function () {
  ...
  if (options.restrictToEmailDomains) {
    try {
      this.restrictedDomains = options.restrictToEmailDomains.split(/[ ,]+/);
      ...
      //      this.membershipRules = new MembershipRules(this);
      this.membershipRules = require('./membership-rules')(this);

      this.unauthorizedDomainUserEntersResponse = 'Sorry, my use is not allowed for all the members in this space. Will ignore any new messages to me.';
      this.unauthorizedDomainStateMessageResponse = 'Sorry, because my use is not allowed for all the members in this space I am ignoring any input.';
      this.unauthorizedDomainUserExitsResponse = 'I am now allowed to interact with all the members in this space and will no longer ignore any input.';
    } catch (e) {
      const msg = `Error: ${e.message}\n` +
        `Unable to initiatilize with config param restrictToEmailDomains: "${options.restrictToEmailDomains}"\n` +
        'Please set to a comma seperated list of valid email domains, ie: "mycompany.com,othercompany.com"';
      return (msg);
    }
  }
  if ("unauthorizedDomainUserEntersResponse" in options) {
    if (!options.restrictToEmailDomains) {
      console.error('Ignoring config param unauthorizedDomainUserEntersResponse, ' +
        'which is only used if restrictToEmailDomains parameter is set.');
    } else {
      this.unauthorizedDomainUserEntersResponse = options.unauthorizedDomainUserEntersResponse;
    }
  }
  if ("unauthorizedDomainStateMessageResponse" in options) {
    if (!options.restrictToEmailDomains) {
      console.error('Ignoring config param unauthorizedDomainStateMessageResponse, ' +
        'which is only used if restrictToEmailDomains parameter is set.');
    } else {
      this.unauthorizedDomainStateMessageResponse = options.unauthorizedDomainStateMessageResponse;
    }
  }
  if ("unauthorizedDomainUserExitsResponse" in options) {
    if (!options.restrictToEmailDomains) {
      console.error('Ignoring config param unauthorizedDomainUserExitsResponse, ' +
        'which is only used if restrictToEmailDomains parameter is set.');
    } else {
      this.unauthorizedDomainUserExitsResponse = options.unauthorizedDomainUserExitsResponse;
    }
  }
  return '';
};
