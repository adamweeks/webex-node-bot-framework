...
var MembershipRules = require('./membership_rules');
...
function Framework(options) {
   ...
   * @property {string} [unauthorizedDomainMessage] - Message from bot when it is added to a space with users whose emails do not match the 
        list specified (if any) in the `restrictedToEmailDomains` parameter.   Default messages is
        "Sorry, my use is not allowed for all the members in this space".
   * 
   */
...
Framework.prototype.onAttachmentActions = function (attachmentAction) {
  ...
  if (bot) {
    return this.getTrigger('attachmentAction', attachmentAction)
      .then((trigger) => {
        this.myEmit('attachmentAction', bot, trigger, this.id);
      });
    // else, bot not found...
  } else {
    // It is is possible that not all rooms we are in were discovered on startup
...
Framework.prototype.spawn = function (membership, actorId) {
  ...
  if (this.options.restrictedToEmailDomains) {
    // Also check that this isn't a "known deactive bot" due to membership rules
    foundBot = _.find(this.inactiveBots, bot => (bot.room.id === roomId));
    if (foundBot) {
      this.debug(`framework.spawn() got a request to spawn in spaceId ${membership.roomId}.  This bot is currently deactived due to membership rules. Ignorning.`);
      return when(false);
    }
  }
...
Framework.prototype.checkMembershipRules = function () {
  ...
  if (options.restrictedToEmailDomains) {
    try {
      ...
      this.membershipRules = new MembershipRules();
      this.unauthorizedDomainMessage = 'Sorry, my use is not allowed for all the members in this space';
    } catch(e) {
  ...
  if (options.unauthorizedDomainMessage) {
    if (!options.restrictedToEmailDomains) {
      console.error('Ignoring config param unauthorizedDomainMessage, ' +
        'which is only used if restrictedToEmailDomains parameter is set.')
    } else {
      this.unauthorizedDomainMessage = options.unauthorizedDomainMessage;
    }
  }
  return '';
};
...
Framework.prototype.myEmit = function (event, ...args) {
  try {
    ...
    if (this.membershipRules) {
      this.membershipRules.emit(this, event, ...args);
    } else {
    ...
    console.error(`Framework.myEmit() failed: ${e.message}. "${event}" event not sent.`);
  }
};
...
Framework.prototype.myEmitWithActor = function (event, bot, actorId) {
  try {
    // TODO there may be some events (ie: log) that I want to always pass on
    if (this.membershipRules) {
      this.membershipRules.emitWithActor(this, event, bot, actorId);
    } else {
    ...
    console.error(`Framework.myEmitWithActor() failed: ${e.message}. "${event}" event not sent.`);
  }
};
...
Framework.prototype.emitBoth = function (event, bot, ...args) {
  try {
    // TODO there may be some events (ie: log) that I want to always pass on
    if (this.membershipRules) {
      this.membershipRules.emitBoth(this, event, bot, ...args);
    } else {
    ...
    console.error(`Framework.emitBoth() failed: ${e.message}. "${event}" event not sent.`);
  }
};
